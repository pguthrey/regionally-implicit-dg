function [correct_cell,maxspeedG] = correct_nortsout(correct_cell,qstar_1D,qnort_1D,qsout_1D,auxiliary,data,cellcenter)
...
for k = 1:data.Pd
    ...
    %Northern Flux
    wstar = data.vectpsi_nort(:,:,tquad,v1quad,v2quad)*qstar_1D;
    wnort = data.vectpsi_sout(:,:,tquad,v1quad,v2quad)*qnort_1D;
    ...
    %Southern Flux
    wstar = data.vectpsi_sout(:,:,tquad,v1quad,v2quad)*qstar_1D;
    wsout = data.vectpsi_nort(:,:,tquad,v1quad,v2quad)*qsout_1D;
    v1 = cellcenter(1)+data.deltav1/2*v1loc;
    v2 = cellcenter(2)-data.deltav2/2;
    v3 = cellcenter(3)+data.deltav3/2*v2loc;
    quadpoint = [v1 v2 v3];
