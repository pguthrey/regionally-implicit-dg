...
for k = 1:data.Pd
    ...
    %Eastern Flux
    wleft = data.vectphi_east(:,:,v1quad,v2quad)*qstar;
    write = data.vectphi_west(:,:,v1quad,v2quad)*qeast;
    ...
    %Western Flux
    write = data.vectphi_west(:,:,v1quad,v2quad)*qstar;
    wleft = data.vectphi_east(:,:,v1quad,v2quad)*qwest;
...
for k = 1:data.Pdp1
    ...
    wstar = data.vectphi(:,:,v1quad,v2quad,v3quad)*qstar;
    phikxii = data.vectphi_dxii_Trans(:,:,v1quad,v2quad,v3quad);
    u = wstar(1);            
    Flux(1,1) = u^2/2 ;
    correct_cell = correct_cell + data.nuv1*phikxii*Flux*weight;
end
