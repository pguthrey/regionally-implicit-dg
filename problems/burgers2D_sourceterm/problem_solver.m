...
if data.smartsolver
    [ searchdir ] = smartsolver(Jacobian,residual,coeffs);
else
...
switch  data.space_dims 
    case 1
        thisresidual = max(abs(residual((1:data.thetaT)+data.thetaT)));%max(abs(residual(coeffs)));
    case 2
        thisresidual = max(abs(residual((1:data.thetaT)+4*data.thetaT)));%max(abs(residual(coeffs)));
    case 3
        thisresidual = max(abs(residual((1:data.thetaT)+13*data.thetaT)));%max(abs(residual(coeffs)));
end
