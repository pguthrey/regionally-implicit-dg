...
for iv1  = 1:(data.Nv1)
    for v1quad = 1:(data.P+2)        
        q_cell = data.vectphi_limiter(:,:,v1quad)*DGcoeffs(:,iv1);
        
        BETA_max(iv1) = max( BETA(iv1) , max(q_max(:,iv1) > M_max(:,iv1)+delta(iv1))); 
        BETA_min(iv1) = max( BETA(iv1) , max(q_min(:,iv1) < M_min(:,iv1)-delta(iv1)));     
    end
end
BETA = BETA+BETA_max+BETA_min;
xmesh = 1:data.Nv1;

DGcoeffs_limited = DGcoeffs;

for iv1  = xmesh(BETA > 0)
    
    DGcoeffs_limited(:,iv1) = data.limiter_filter_phi*DGcoeffs(:,iv1);
end

problem_plotting_whilerunning(DGcoeffs_limited,data.Time,data)
...
%{
    debug_plot_DGphi(DGcoeffs_limited,data)    
    debug_plot_DGphi(DGcoeffs,data)
%}
%{
