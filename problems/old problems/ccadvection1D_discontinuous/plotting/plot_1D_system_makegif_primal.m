function [filename1] = plot_1D_system_makegif_primal(solution,data)
...
filename1 =[path filename '_primal.gif'];
...
for iv1 = 1:data.Nv1
    ...
    for ixii = 1:data.P
        ...
        DGplot(:,ixii) = problem_cons2prim(temp);
    end    
    for eqn = 1:data.Neqns
        subplot(1,data.Neqns,eqn)
        zplot(:,1) = DGplot(eqn,:);
        plot(v1plot,zplot,'-b')
		axis([data.boundsv1v2 data.boundsq(eqn,1:2)])
        hold on
    end
end
