function [ DGcoeffs_limited ] = limiter_DGlimiter(DGcoeffs,data)
%2D Limiter
...
for iv2  = 1:(data.Nv2)
    ...
    for v2quad = 1:(data.P+2) 
        qcell_quad = data.vectphi_limiter(:,:,v1quad,v2quad)*DG_ghosts(:,iv1,iv2);
        q_min_cell = min(q_min_cell,qcell_quad);
        q_max_cell = max(q_max_cell,qcell_quad);
    end
    end
    q_ave(:,iv1,iv2) = data.vectphi_average*DG_ghosts(:,iv1,iv2);
...
for iv2  = 1:(data.Nv2)
    ...
    data.M_max(:,iv1,iv2) = max(Qneighborsmax,[],2);
    data.M_min(:,iv1,iv2) = min(Qneighborsmin,[],2);
end
end
...
limtol = 1e-8;
maxprecut = abs(data.M_max-q_ave)./(abs(q_max-q_ave)+limtol);
minprecut = abs(data.M_min-q_ave)./(abs(q_min-q_ave)+limtol);
...
inner_prod_average = data.vectphi_average'*data.vectphi_average;
...
%{
N = 2^data.space_dims;
highermoments = (eye(data.theta)-inner_prod_average);
