...
for k = 1:Pdm1
    ...
    u = wleft(1);
    F_left(1,1) = data.appdata.nuf*u;      
    speed = abs(data.appdata.nuf);

    u = write(1);

    F_rite(1,1) = data.appdata.nuf*u ;   
    speed = max(speed,abs(data.appdata.nuf));
    ...
    correct_cell = correct_cell + (nuv1*weight)*phik*Flux;
...
for k = 1:Pd
    ...
    u = wstar(1);            
    Flux(1,1) = data.appdata.nuf*u ;
    correct_cell = correct_cell - phikxii*Flux*(nuv1*weight);
end
