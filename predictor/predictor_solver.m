...
coeffs = (1:thetaT)+(main_cell-1)*thetaT;

%{
predictor_backslash_solver(Jacobian,residual,thetaT);
predictor_RI1DG_solver(Jacobian,residual,thetaT);
predictor_RI1DG_solver_augmented(Jacobian,residual,thetaT);
...
DGprediction_cell = DGprev_cell - searchdir;

thisresidual = max(abs(residual((1:data.thetaT)+(main_cell-1)*data.thetaT)));

%{
if thisresidual > .4%any(any(any(isnan(DGprediction_cell))))
    close all
    subplot(1,2,1)
    plot(residual)
    subplot(1,2,2)
    surf(Jacobian)
    axis([1 9*thetaT 1 9*thetaT])
    keyboard
end
%}
